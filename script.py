...
def safe_print(message):
    """Safe print function that flushes output immediately"""
    try:
        print(message)
        sys.stdout.flush()
    except:
...
class IntegratedOrderProcessor:
    def __init__(self):
        ...
        if not os.path.exists(self.shots_folder):
            ...
            safe_print(f"📁 Created '{self.shots_folder}' folder for screenshots")
    ...
    def run_opening_script(self):
        ...
        safe_print("🚀 Running opening sequence...")
        ...
        try:
            ...
            for config in configs:
                try:
                    ...
                    safe_print(f"  OCR text: '{text.strip()}'")
                    ...
                    for pattern in patterns:
                        ...
                        if matches:
                            ...
                            if 8 <= len(candidate) <= 10:
                                ...
                                safe_print(f"  Found order number: {candidate}")
                                break
                    ...
                    try:
                        ...
                        for pattern in fallback_patterns:
                            ...
                            if numbers:
                                ...
                                safe_print(f"  Found fallback number: {best_number}")
                                break
            ...
            if best_number:
                safe_print(f"✅ Extracted order number: {best_number}")
                return best_number
            else:
                safe_print("❌ Could not extract order number")
                return None
                
        except Exception as e:
            safe_print(f"❌ Order extraction failed: {e}")
            traceback.print_exc()
            return None
    
    def run_delete_sequence(self):
        """Execute delete.py functionality"""
        safe_print("🗑️ Running delete sequence...")
        ...
        try:
            ...
            safe_print(f"  1. Clicking at ({coordinates[0]['x']}, {coordinates[0]['y']})")
            pyautogui.click(coordinates[0]['x'], coordinates[0]['y'])
            time.sleep(5)
            
            # Step 2: Ctrl + A (Select All)
            safe_print("  2. Pressing Ctrl+A (Select All)")
            pyautogui.hotkey('ctrl', 'a')
            time.sleep(0.2)
            
            # Step 3: Delete
            safe_print("  3. Pressing Delete")
            pyautogui.press('delete')
            time.sleep(0.2)
            
            # Step 4: Tab
            safe_print("  4. Pressing Tab")
            pyautogui.press('tab')
            time.sleep(0.2)
            
            # Step 5: Enter
            safe_print("  5. Pressing Enter")
            pyautogui.press('enter')
            time.sleep(0.3)
            
            # Step 6: Click on second coordinate
            safe_print(f"  6. Clicking at ({coordinates[1]['x']}, {coordinates[1]['y']})")
            pyautogui.click(coordinates[1]['x'], coordinates[1]['y'])
            time.sleep(0.3)
            
            # Step 7: Tab (after second click)
            safe_print("  7. Pressing Tab (after second click)")
            pyautogui.press('tab')
            time.sleep(0.2)
            
            # Step 8: Enter (after second click)
            safe_print("  8. Pressing Enter (after second click)")
            pyautogui.press('enter')
            time.sleep(0.3)
            
            # Step 9: Move mouse randomly
            safe_print("  9. Moving mouse slightly...")
            ...
            safe_print("✅ Delete sequence completed!")
            time.sleep(1)  # Wait for deletion to process
            return True
            
        except Exception as e:
            safe_print(f"❌ Delete sequence failed: {e}")
            traceback.print_exc()
            return False
    ...
    def load_orders_from_json(self):
        ...
        try:
            with open('main/filtered_positions.json', 'r') as f:
                ...
                safe_print(f"📄 Loaded main/filtered_positions.json")
                ...
                safe_print(f"  Found {len(filtered_data)} orders in filtered_positions.json")
                            
        except FileNotFoundError:
            safe_print("⚠️ main/filtered_positions.json not found")
        except Exception as e:
            safe_print(f"❌ Error loading main/filtered_positions.json: {e}")
            traceback.print_exc()
        ...
        try:
            with open('main/total_trades.json', 'r') as f:
                ...
                safe_print(f"📄 Loaded main/total_trades.json")
                ...
                safe_print(f"  Found {len(trades_data)} orders in total_trades.json")
                            
        except FileNotFoundError:
            safe_print("⚠️ main/total_trades.json not found")
        except Exception as e:
            safe_print(f"❌ Error loading main/total_trades.json: {e}")
            traceback.print_exc()
        
        orders_list = sorted(list(orders))
        safe_print(f"📊 Total unique orders loaded: {len(orders_list)}")
        
        if orders_list:
            safe_print(f"📋 Sample orders: {orders_list[:5]}...")
            if len(orders_list) > 5:
                safe_print(f"    ... and {len(orders_list) - 5} more")
    ...
    def click_and_screenshot(self):
        ...
        safe_print(f"Clicked at ({click_coords[0]}, {click_coords[1]})")
        pyautogui.sleep(1)
        ...
        safe_print(f"Screenshot saved: {filepath}")
    ...
    def double_click_order(self, order_location, click_offset_x=50):
        ...
        safe_print(f"  🖱️ Double-clicking BESIDE order at ({click_x}, {click_y}) [offset: -{click_offset_x}px]")
    ...
    def verify_order_match(self, clicked_order, detected_order):
        ...
        if clicked_order == detected_order:
            safe_print(f"✅ Perfect match: {clicked_order} = {detected_order}")
            return True
        elif clicked_order in detected_order or detected_order in clicked_order:
            safe_print(f"✅ Partial match: {clicked_order} ≈ {detected_order}")
            return True
        else:
            safe_print(f"❌ No match: {clicked_order} ≠ {detected_order}")
            return False
    ...
    def process_single_order(self, target_order, screenshot):
        ...
        if not found_orders:
            safe_print(f"❌ Order {target_order} not found in current view")
            return False
        
        best_match = max(found_orders, key=lambda x: x['confidence'])
        safe_print(f"📍 Found order location: {best_match['text']}")
        
        # Step 1: Double-click on the order
        safe_print("1️⃣ Double-clicking on order...")
        if not self.double_click_order(best_match):
            safe_print("❌ Failed to double-click order")
            return False
        
        # Step 2: Extract order number from opened interface
        safe_print("2️⃣ Extracting order number from interface...")
        detected_order = self.extract_order_number_from_interface()
        
        # Step 3: Verify match
        safe_print("3️⃣ Verifying order match...")
        if not self.verify_order_match(target_order, detected_order):
            safe_print(f"❌ Order mismatch - skipping deletion")
        ...
        safe_print("4️⃣ Running delete sequence...")
        if self.run_delete_sequence():
            safe_print(f"✅ Successfully processed order: {target_order}")
            self.processed_orders.add(target_order)
            return True
        else:
            safe_print(f"❌ Failed to delete order: {target_order}")
            return False
    
    def integrated_workflow(self):
        """Main integrated workflow"""
        safe_print("🚀 INTEGRATED ORDER PROCESSING WORKFLOW")
        safe_print("=" * 70)
        
        # Step 1: Run opening script
        if not self.run_opening_script():
            safe_print("❌ Opening script failed - aborting")
            return False
        
        # Step 2: Load target orders
        self.target_orders = self.load_orders_from_json()
        if not self.target_orders:
            safe_print("❌ No target orders loaded - aborting")
            return False
        ...
        while True:
            ...
            if not remaining_orders:
                ...
                safe_print(f"Successfully processed all {len(self.processed_orders)} orders!")
                return True
            ...
            safe_print(f"📋 Current view has {len(current_orders)} orders")
            safe_print(f"🎯 Remaining targets: {len(remaining_orders)}")
            ...
            if previous_orders and set(current_orders) == set(previous_orders):
                ...
                safe_print(f"\n📊 WORKFLOW COMPLETED!")
                safe_print(f"Processed: {len(self.processed_orders)}/{len(self.target_orders)} orders")
                
                if remaining > 0:
                    safe_print(f"❌ {remaining} orders not found:")
            ...
            safe_print("⬆️ Navigating up to find more orders...")
    ...
    def navigate_up_intelligently(self, current_orders, previous_orders):
        ...
        safe_print(f"📊 Order Analysis:")
        if previous_orders:
            new_orders = set(current_orders) - set(previous_orders)
            safe_print(f"  New orders: {len(new_orders)}")
            safe_print(f"  Old orders: {len(set(current_orders) & set(previous_orders))}")
            safe_print(f"  Total current: {len(current_orders)}")
        
        # Step 1: First select the top order (click beside it to select)
        if current_orders:
            safe_print(f"🎯 Selecting top order for navigation...")
            ...
            if found_orders:
                ...
                safe_print(f"  Clicking beside top order {top_order} at ({click_x}, {click_y}) to select it")
                pyautogui.click(click_x, click_y)
                pyautogui.sleep(0.5)
            else:
                safe_print(f"  Could not locate top order {top_order} - using direct navigation")
        
        # Step 2: Now navigate up using arrow keys
        safe_print(f"⬆️ Moving up {moves} positions...")
        for i in range(moves):
            pyautogui.press('up')
            if i % 5 == 4:  # Show progress every 5 moves
                safe_print(f"  Progress: {i+1}/{moves} moves completed")
            time.sleep(0.1)
        
        safe_print(f"✓ Completed {moves} UP movements")
...
def main():
    safe_print("🚀 Integrated Order Processing System")
    safe_print("=" * 50)
    safe_print("Workflow:")
    safe_print("1. 🚀 Opening script")
    safe_print("2. 🔍 Search and double-click orders")
    safe_print("3. 🔍 Detect order number")
    safe_print("4. ✅ Verify match")
    safe_print("5. 🗑️ Delete if match")
    safe_print("6. 🔄 Continue to next order")
    safe_print("=" * 50)
    
    processor = IntegratedOrderProcessor()
    
    try:
        safe_print("Starting integrated workflow...")
...
if __name__ == "__main__":
    try:
        ...
        safe_print(f"Fatal error in main: {e}")
        traceback.print_exc()
        sys.exit(1)